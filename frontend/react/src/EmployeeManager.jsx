...
function EmployeeManager() {
  ...
  const addEmployee = async (e) => {
    e.preventDefault();
    await axios.post("http://localhost:8000/add-emp", formData);
  ...
  const getEmployee = async () => {
    try {
      const res = await axios.get(`http://localhost:8000/singleemp/${searchId}`);
  ...
  const updateEmployee = async () => {
    await axios.put(
      `http://localhost:8000/updateemp/${singleEmployee._id}`,
      formData
    );
  ...
  const deleteEmployee = async () => {
    await axios.delete(`http://localhost:8000/deleteemp/${singleEmployee._id}`);
